...
## [Unreleased]
### Added
- Added `IterableAPI.switchProject(project:callback:)` for apps that need to move a running install from one Iterable project to another (for example a multi-region app moving between a US and an EU project) without an app restart. The method returns immediately and runs the whole sequence off the calling thread: it disables the push token on the previous project, resets that project's in-app and embedded state, purges its persisted offline queue, clears its identity from memory and from storage (email, user ID, unknown-user ID and auth token) along with the rest of that project's storage (activation criteria, unsent unknown-user events, sessions and updates, and stored attribution info), and then stands up a fresh instance against the new API key and config. SDK calls made between the call and the callback are queued and then run in order against the new project.
  - The callback is delivered on the main thread and receives an `IterableProjectSwitchResult`, either `.switchedCleanly` or `.switchedWithWarnings`. Both mean the SDK **is** on the new project; they differ only in whether leaving the previous one was tidy, and neither means the switch failed or was rolled back. A named result rather than a boolean because the meaning of a bare `false` had to be explained everywhere it appeared. The SDK reports `.switchedWithWarnings` when push registration is off, when there is no device token to disable, when no user was identified on the previous project, or when the device disable request fails, so for an app that does not use push it is the expected outcome and not an error: carry on and re-identify the user exactly as you would after `.switchedCleanly`. The callback is not held open for the device disable response, so a disable that fails after the callback has been delivered is logged rather than reported retroactively.
  - `switchProject` does not re-identify the user. Call `setEmail`/`setUserId` from the callback. The JWT auth retry budget does not carry over: it is state on the `AuthManager` instance, and the switch builds a fresh one along with the rest of the dependency container.
  - Added `IterableProject`, which pairs a project's API key with the `IterableConfig` to run it with. `switchProject` takes one of these rather than a loose key and config, because `IterableConfig` carries the data region and the `authDelegate`: supplying the two halves separately allows one project's key to be paired with another project's region or auth delegate, and that mismatch produces an authentication failure, or a request sent to the wrong region, that looks unrelated to the project it came from. Its initializer is failable and returns `nil` for an empty or whitespace-only key, so an unusable project cannot be built and a blank key can no longer reach a teardown. Note that `IterableConfig` holds its delegates weakly, so an app keeping a project as a long-lived constant should keep a strong reference to its delegates too.
  - Called before any `initialize`, it behaves as `initialize` and logs a warning, and reports `.switchedCleanly` once that initialization has started: there is no previous project, so no teardown step could have been noisy. Android reports the same.
  - Repeated calls are resolved against the project most recently *asked* for rather than the one currently live, so the SDK always ends up where the app last asked to be. A request for the project already asked for is absorbed: with nothing in flight it is a no-op reporting `.switchedCleanly`, and while a switch to it is running its callback joins that switch rather than starting a second teardown, so a picker tapped twice on one destination runs a single teardown. Anything else is queued and runs in the order it was asked for, each callback firing when the project it asked for is live. Asking to go back to the project a running switch is leaving is therefore a real switch and not a no-op, and `A → B → A` reports B and then A and ends on A. Between two switches in a chain the SDK is fully live on the project that just landed, so calls made from its callback, re-identifying the user in particular, run against that project instead of being replayed into the next one.
  - Queued `users/disableDevice` requests survive the purge, so the token is still disabled on the project being left even when the switch happens offline. The device ID is project-agnostic and is preserved. The switch waits briefly, and with a bound, for that disable to reach the request layer before it releases the previous project's instance, because a request built with the previous API key cannot be sent once that instance is gone; it does not wait for the network response.
  - Calls that carry the previous project's campaign, template or message IDs are **not** queued: push handling, in-app and embedded tracking, `inAppConsume`, `updateSubscriptions` and inbox sessions run against the project the SDK is on when they are made. Neither are calls where deferral is itself the problem: device disables, visitor consent, device attributes and `pauseAuthRetries`. Identity and generic tracking (`setEmail`, `setUserId`, `updateEmail`, `updateUser`, `updateCart`, `trackPurchase`, `track`, `registerToken`, `logoutUser`) are queued and replayed.
...
- Calls queued during a project switch can no longer be overtaken by a call that arrives as the switch lands. The gate was lowered before the queue was replayed, so a `setEmail` made at that moment could run first and then be overwritten by the older one replayed behind it. The gate now stays raised until the replay is finished.
...
- The check that stops a deep link redirect from writing the previous project's attribution back after a switch is now performed under a lock together with the write. Reading the flag and then writing outside it left a window in which a redirect could pass the check, have the switch clear attribution, and then put the previous project's campaign back into the storage the new project reads.
