...
# Process XCResult into HTML file, JSON summary and Markdown
if [ "$OPEN_IN_BROWSER" = "true" ]; then
  python scripts/process_xcresult.py \
    --path "$XCRESULT_PATH" \
    --output "$OUTPUT_DIR/test-results.html" \
    --summary-json "$OUTPUT_DIR/test-summary.json" \
    --markdown-output "$OUTPUT_DIR/report-summary.md" \
    --open-in-browser
else
  python scripts/process_xcresult.py \
    --path "$XCRESULT_PATH" \
...
# Create HTML summary and details for GitHub Checks
head -c 10000 "$OUTPUT_DIR/test-results.html" > "$OUTPUT_DIR/report-summary.html"
cp "$OUTPUT_DIR/test-results.html" "$OUTPUT_DIR/report-detail.html"

# If for some reason the markdown file wasn't created, create a simple one
...
# Read values from JSON for console output
if [ "$USE_JQ" = true ]; then
  # Use jq if available
  TOTAL_TESTS=$(jq -r '.total_tests' "$OUTPUT_DIR/test-summary.json")
  PASSED_TESTS=$(jq -r '.passed_tests' "$OUTPUT_DIR/test-summary.json")
