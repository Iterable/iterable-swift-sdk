...
final class BackendStatusViewController: UIViewController {
    ...
    private let sendDeepLinkPushButton: UIButton = {
        let button = UIButton(type: .system)
        button.setTitle("Send Deep Link Push (Campaign \(BCITCampaign.deepLinkPush))", for: .normal)
    ...
    private let sendSilentPushButton: UIButton = {
        let button = UIButton(type: .system)
        button.setTitle("Send Silent Push (Campaign \(BCITCampaign.silentPush))", for: .normal)
    ...
    @objc private func sendDeepLinkPushNotification() {
        ...
        pushSender.sendDeepLinkPush(to: testUserEmail, campaignId: BCITCampaign.deepLinkPush) { [weak self] success, messageId, error in
            DispatchQueue.main.async {
                self?.sendDeepLinkPushButton.isEnabled = true

                if success {
                    let message = "Deep link push notification sent successfully!\nCampaign ID: \(BCITCampaign.deepLinkPush)"
                    if let messageId = messageId {
    ...
    @objc private func sendSilentPushNotification() {
        ...
        pushSender.sendSilentPush(to: testUserEmail, campaignId: BCITCampaign.silentPush) { [weak self] success, messageId, error in
            DispatchQueue.main.async {
