...
class EmbeddedMessagesViewController: UIViewController {
    ...
    func loadCardView(_ embeddedView: IterableEmbeddedView, _ embeddedMessage: ResolvedMessage) {
        DispatchQueue.main.async { [self] in
            loadEmbeddedView(embeddedView, embeddedMessage: embeddedMessage, type: "card")
        }
    ...
    func loadBannerView(_ embeddedMessage: ResolvedMessage) {
        DispatchQueue.main.async { [self] in
            loadEmbeddedView(embeddedBannerView, embeddedMessage: embeddedMessage, type: "banner")
        }
        
    }
    
    func loadEmbeddedView(_ embeddedView: IterableEmbeddedView, embeddedMessage: ResolvedMessage, type: String) {
        embeddedView.iterableEmbeddedViewDelegate = self
        embeddedView.bannerCornerRadius = 10
        ...
        embeddedView.configure(title: embeddedMessage.title, description: embeddedMessage.description, image: embeddedMessage.image, buttonText: embeddedMessage.buttonText, buttonTwoText: embeddedMessage.buttonTwoText, message: embeddedMessage.message, style: type)
