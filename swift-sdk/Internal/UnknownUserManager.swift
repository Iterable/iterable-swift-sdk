...
public class UnknownUserManager: UnknownUserManagerProtocol {
    ...
    /// Stores a unknown user sessions locally. Updates the last session time each time when new session is created
    ...
    private func createUnknownUser(_ criteriaId: String) {
        var anonSessions = convertToDictionary(data: localStorage.unknownUserSessions?.itbl_unknown_user_sessions)
        ...
        notificationStateProvider.isNotificationsEnabled { isEnabled in
            ...
            IterableAPI.implementation?.apiClient.trackUnknownUserSession(
                ...
                self.localStorage.userIdUnknown = userId
                self.config.unknownUserDelegate?.onUnknownUserCreated(userId: userId)
    ...
    /// Checks if criterias are being met and returns criteria id if it matches the criteria.
    private func evaluateCriteriaAndReturnID() -> String? {
        ...
        if let anonymousUserEvents = localStorage.unknownUserEvents {
    ...
    private func storeEventData(type: String, data: [AnyHashable: Any], shouldOverWrite: Bool = false) {
        ...
        if !self.localStorage.unknownUsageTrack {
