...
final class InternalIterableAPI: NSObject, PushTrackerProtocol, AuthProvider {
    ...
    var userId: String? {
        get {
            ...
            setUserId(newValue, merge: false)
        }
    }
    ...
    func setEmail(_ email: String?, authToken: String? = nil, successHandler: OnSuccessHandler? = nil, failureHandler: OnFailureHandler? = nil) {
        ...
        anonymousUserMerge.tryMergeUser(sourceUserId: localStorage.userIdAnnon, sourceEmail: nil, destinationUserId: nil, destinationEmail: email, merge: false) { mergeResult, error in
    ...
    func getMergeDefaultValue(merge: Bool?) -> Bool {
        ...
        print("vvvv isEmailOrUserId \(isEmailOrUserId)")
        print("vvvv anonUser22 \(anonUser)")
        if let merge = merge {
               return merge
           } else {
               if (!isEmailOrUserId && anonUser == nil) { // Criteria is not yet met (default merge is true)
                return true;
            } else if (isEmailOrUserId && anonUser != nil) { // Criteria is met (Iterable profile created with an autogenerated identity)(default merge is true)
                return true;
            } else { // Current logged in user is identified (default merge is false)
                return false;
            }
        }
    }
    func getSourceUserIdOrEmail()  -> (sourceUserId: String?, sourceEmail: String?){
        ...
        print("vvvv userIdLocal \(userIdLocal)")
        print("vvvv emailLocal \(emailLocal)")
        print("vvvv anonUserIdLocal \(anonUserIdLocal)")
//        var typeOfAuth = "userId";
        var sourceUserId: String? = nil
        var sourceEmail: String? = nil

//        if (emailLocal == nil && userIdLocal != nil) {
//            typeOfAuth = "userId"
//        }
//        else if (emailLocal != nil && userIdLocal == nil) {
//            typeOfAuth = "email"
//        }
    ...
    func setUserId(_ userId: String?, merge: Bool?, authToken: String? = nil, successHandler: OnSuccessHandler? = nil, failureHandler: OnFailureHandler? = nil) {
        ITBInfo()

        let isMerge = getMergeDefaultValue(merge: merge);
        let (sourceUserId, sourceEmail) = getSourceUserIdOrEmail();
        anonymousUserMerge.tryMergeUser(sourceUserId: sourceUserId, sourceEmail: sourceEmail, destinationUserId: userId, destinationEmail: nil, merge: isMerge) { mergeResult, error in
            if mergeResult == MergeResult.mergenotrequired ||  mergeResult == MergeResult.mergesuccessful {
