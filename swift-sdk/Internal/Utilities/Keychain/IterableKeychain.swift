...
class IterableKeychain {
    ...
    var userIdUnknown: String? {
        get {
            let data = wrapper.data(forKey: Const.Keychain.Key.userIdUnknown)
        ...
        set {
            guard let token = newValue,
                  let data = token.data(using: .utf8) else {
                wrapper.removeValue(forKey: Const.Keychain.Key.userIdUnknown)
                return
            }
            
            wrapper.set(data, forKey: Const.Keychain.Key.userIdUnknown)
        }
    }
